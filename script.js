(() => {
  ...
  // Tu función original intacta para el polvo mágico interno
  ...
  // Coordenadas manuales de tus destellos
  ...
  // Generador automático de estrellas de fondo del universo
  const starCount = 80;
  for (let i = 0; i < starCount; i++) {
    ...
    star.style.top = `${Math.random() * 55}vh`; // Se generan solo arriba de la curva del planeta
    star.style.animationDelay = `${Math.random() * 4}s`;
    star.style.animationDuration = `${Math.random() * 3 + 2}s`;
    castle.appendChild(star);
  }
